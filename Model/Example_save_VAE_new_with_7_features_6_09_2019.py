...
# import Age_Prediction_restore as Age_Classifier
...
def encoder(X_in, X_details, keep_prob):
    ...
    with tf.variable_scope("vae_encoder", reuse=None):
        X = tf.reshape(X_in, shape=[-1, heigh_global, width_global, 3], name="vae_encoder_input_reshape")
        ...
        mn = tf.layers.dense(x, units=n_latent, name="vae_encoder_mean")
        sd = 0.5 * tf.layers.dense(x, units=n_latent, name="vae_encoder_std")
        epsilon = tf.random_normal(tf.stack([tf.shape(x)[0], n_latent]), name="vae_encoder_epsilon")
...
def decoder(sampled_z, keep_prob):
    with tf.variable_scope("decoder", reuse=None):
        x = tf.layers.dense(sampled_z, units=inputs_decoder, activation=lrelu, name="vae_decoder_x_1")
        x = tf.layers.dense(x, units=inputs_decoder * 2, activation=lrelu, name="vae_decoder_x_2")
        ...
        x = tf.contrib.layers.flatten(x)
        x = tf.layers.dense(x, units=heigh_global * width_global * 3, activation=tf.nn.sigmoid, name="vae_decoder_x_6")
        img = tf.reshape(x, shape=[-1, heigh_global, width_global, 3], name="vae_decoder_img")
        return img
...
if __name__ == '__main__':
    ...
    # loss_Age = tf.placeholder(dtype=tf.float32, shape=(), name='lossAge')
    ...
    unreshaped = tf.reshape(dec, [-1, heigh_global*width_global * 3], name="vae_img_flatten") # flatten the code
    img_loss = tf.reduce_sum(tf.squared_difference(unreshaped, Y_flat), 1, name="vae_img_loss")
    latent_loss = -0.5 * tf.reduce_sum(1.0 + 2.0 * std - tf.square(mean) - tf.exp(2.0 * std), 1, name="vae_latent_loss")
    # loss = tf.reduce_mean(img_loss + latent_loss + loss_Age, name="vae_loss")
    ...
    with tf.Session() as sess:
        ...
        for i in range(150):
            ...
            #     k = 0
                ...
                # age = Age_Classifier.agePrediction(imgs)
                ...
                if k % 1 == 0:
                    ...
                    if min_loss > local_loss * 1.1 and local_loss < 150:
                        min_loss = local_loss
                        os.makedirs("model_"+str(min_loss)+"_epoch"+str(i))
                        saved_path = saver.save(sess, "model_"+str(min_loss)+"_epoch"+str(i)+'/saved_variable_VAE_Faces_56_06092019')

                #     # print("Real image")
                #     # print("\n")
                #     #
                #     # batch = batch.reshape([-1, 56, 56])
                #     # plt.imshow(batch[0], cmap='gray', interpolation='bicubic')
                #     # plt.show()
                #     #
                #     # print("Fake image")
                #     # print("\n")
                #     #
                #     # imgs = imgs.reshape([-1, 56, 56])
                ...
                #     batch = batch.reshape([-1, 56, 56, 3])
                #     details = np.array(oneBatch[0])
                #     print("The real details are: \n\n")
                #     # print(details)
                #     details = details.reshape([-1, 7])
                #     # os.mkdir(str(i) + "_" + str(details) + "_VAE")
                #
                #     batch_img = [batch[0] for _ in range(1, 101, int(20))]
                #     details_img = [[year, 1, 1, 0, 0, 0, 0] for year in range(1, 101, int(20))]
                #
                #     # Print the loaded variable
                #     imgs = np.reshape(np.array(sess.run([unreshaped], feed_dict={X_in: batch_img, X_detail: details_img, keep_prob: 1.0})), [5, 56, 56, 3])

                    # for j in range(len(batch_img)):
                    #     im = np.array(imgs[j])
                    #     im = im * 255.0
                    #     f = np.array(im).astype(np.uint8)
                    #     # cv2.imwrite(str(i) + "_" + str(details) + "_VAE/" + str(years_between_images * j) + "_1.png", im)
                    #     # x = np.array([[1, 2, 3], [4, 5, 6]], np.int32)
                    #     # plt.imshow(f)
                    #     # f = np.reshape(np.array(f), [1, 56, 56])
